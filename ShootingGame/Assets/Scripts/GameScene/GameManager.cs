...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] TMP_Text textGameOverBtn;
    ...
    public void GameOver()
    {
        ...
        for(int iNum = 0; iNum < count; iNum++)
        {
            ...
            if(userData.Score < score)//내 점수가 랭크보다 높다면 해당 랭크를 차지 해야함
            {
                rank = iNum;
                break;
            }
        ...
        if(rank != -1)
        {
            ...
            textGameOverBtn.text = "등록";
        }
        else //랭크 안에 들지 못했다면 이름을 적을 필요가 없음
        {
            textGameOverMenuRank.text = "랭킹에 들지 못했습니다.";
            IFGameOverMenuRank.gameObject.SetActive(false);
            textGameOverBtn.text = "메인메뉴로";
        }

        //위의 if문을 아래 코드로 축약할 수 있음
        //textGameOverMenuRank.text = rank != -1 ? $"랭킹 : {rank + 1}등" : "랭킹에 들지 못했습니다.";
        //IFGameOverMenuRank.gameObject.SetActive(rank != -1);
        //textGameOverBtn.text = rank != -1 ? "등록" : "메인메뉴로";
